...
int main(int argc, char **argv) {
	...
	printf("%jd < int8_t < %jd\n", (intmax_t)INT8_MIN, (intmax_t)INT8_MAX);
	printf("0 < uint8_t < %ju\n", (intmax_t)UINT8_MAX);
	printf("\n");

	printf("%jd < int16_t < %jd\n", (intmax_t)INT16_MIN, (intmax_t)INT16_MAX);
	printf("0 < uint16_t < %ju\n", (intmax_t)UINT16_MAX);
	printf("\n");

	printf("%jd < int32_t < %jd\n", (intmax_t)INT32_MIN, (intmax_t)INT32_MAX);
	printf("0 < uint32_t < %ju\n", (intmax_t)UINT32_MAX);
	printf("\n");

	printf("%jd < int64_t < %jd\n", (intmax_t)INT64_MIN, (intmax_t)INT64_MAX);
	printf("0 < uint64_t < %ju\n", (intmax_t)UINT64_MAX);
	printf("\n");

	printf("%lld < intmax_t < %lld\n", INTMAX_MIN, INTMAX_MAX);
	printf("0 < uintmax_t < %llu\n", UINTMAX_MAX);
